...
namespace PegasusPlus.Controllers.DataControllers
{
    public class AdminController : Controller
    {
        ...
        public List<sqlTeacherAitiseis> GetAitiseisAllFromDB()
        {
            ...
            if (prokirixiId > 0)
            {
                data = (from d in db.sqlTeacherAitiseis where d.ProkirixisID == prokirixiId orderby d.FullName, d.SchoolName select d).ToList();
            }
            return (data);
        }
        ...
        public bool AitiseisExist(int prokirixiId)
        {
            var data = (from d in db.sqlTeacherAitiseis
                          where d.ProkirixisID == prokirixiId
                          select d).Count();
        ...
        public ActionResult EditPersonal(int aitisiId)
        {
            ...
            if (prokirixiId == 0)
            {
                return RedirectToAction("Index", "Admin", new { notify = "Δεν βρέθηκε ενεργοποιημένη προκήρυξη για διαχείριση." });
            }
        ...
        public ActionResult EditSkills(int aitisiId = 0)
        {
            ...
            if (c.GetAdminProkirixiID() == 0)
            {
                return RedirectToAction("Index", "Admin", new { notify = "Δεν βρέθηκε ανοικτή προκήρυξη για επεξεργασία." });
            }
        ...
        public ActionResult TeacherUploadsAdmin(string afm = null)
        {
            bool val1 = (System.Web.HttpContext.Current.User != null) && System.Web.HttpContext.Current.User.Identity.IsAuthenticated;
            if (!val1)
            {
        ...
        public ActionResult EpimorfoseisAdmin(string afm = null)
        {
            //check if user is unauthenticated to redirect him
            bool val1 = (System.Web.HttpContext.Current.User != null) && System.Web.HttpContext.Current.User.Identity.IsAuthenticated;
            if (!val1)
        ...
        public ActionResult SuperNova()
        {
            ...
            var aitiseis = (from d in db.Aitisis where d.ProkirixisID == prokirixiId select d).ToList();
            if (aitiseis.Count == 0)
            {
                string msg = "Δεν βρέθηκαν αιτήσεις γι' αυτή την Προκήρυξη. Η μοριοδότηση ακυρώθηκε.";
                return RedirectToAction("Index", "Admin", new { notify = msg });
            }
            ...
            string message = "Η μοριοδότηση όλων των αιτήσεων της προκήρυξης ολοκληρώθηκε.";
        ...
        public ActionResult TeachersRegistry()
        {
            ...
            if (!(prokirixiId > 0))
            {
                string msg = "Δεν βρέθηκε ενεργοποιημένη διαχειριστική προκήρυξη";
                return RedirectToAction("Index", "Admin", new { notify = msg });
            }
        ...
        // Assumes aitiseis exist! Check before using this
        public sqlTeacherAitiseis GetSelectedAitisi()
        {
            prokirixiId = c.GetAdminProkirixiID();
